...
DEFAULT_SOPS_AGE_KEYS_FILE = Path.home() / ".config" / "sops" / "age" / "keys.txt"
...
def _validate_age_identity_configured() -> None:
    # SOPS uses SOPS_AGE_KEY_FILE if set; otherwise it defaults to ~/.config/sops/age/keys.txt.
    # Fail fast with a clear message before we do any clone/decrypt work.
    key_file_env = os.environ.get("SOPS_AGE_KEY_FILE", "").strip()
    if key_file_env:
        key_file = Path(key_file_env).expanduser()
        if key_file.is_file():
            return
        raise RuntimeError(
            "SOPS_AGE_KEY_FILE is set but the file does not exist: "
            f"{key_file}\n"
            "Fix your age key configuration. See DEVELOPMENT.md -> Secrets bootstrap -> Developer setup scenarios (age key)."
        )

    if DEFAULT_SOPS_AGE_KEYS_FILE.is_file():
        return

    raise RuntimeError(
        "Missing age identity file for SOPS. Expected one of:\n"
        f"- {DEFAULT_SOPS_AGE_KEYS_FILE}\n"
        "- or set SOPS_AGE_KEY_FILE to point to your age private key\n\n"
        "See DEVELOPMENT.md -> Secrets bootstrap -> Developer setup scenarios (age key)."
    )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Generate deceptgold _secrets_generated.py from a SOPS-encrypted secrets file"
    )
    parser.add_argument(
        "--secrets-file",
    ...
    parser.add_argument(
        ...
        help=(
            "Path to local secrets repo folder (default: env DECEPTGOLD_SECRETS_REPO). "
            "If provided, secrets-file defaults to <repo>/secrets.enc.json"
        ),
    )
    parser.add_argument(
        "--secrets-repo-url",
    ...
    _validate_age_identity_configured()
...
def cli_main() -> int:
    try:
        ...
        # Keep output short and actionable (no stacktrace).
        print("ERROR: Bootstrap failed.")
        print("See: DEVELOPMENT.md -> Secrets bootstrap -> Developer setup scenarios (age key).")
        return 1
